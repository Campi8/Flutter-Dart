...
class Quantidade extends StatefulWidget {
  ...
  const Quantidade({
    super.key,
    required this.onChangeQuantidade,
    required this.min,
    required this.max
    });
...
class _QuantidadeState extends State<Quantidade> {
  ...
  void incrementa(){
    ...
    if (quantidade > widget.max) {
      final snackBar = SnackBar(content: Text("Não é possivel comprar mais que ${widget.max} produtos"),
      );
  ...
  void decrementa(){
    ...
    if (quantidade < widget.min) {
      const snackBar = SnackBar(content: Text("Não se pode comprar quantidade negativa"),
      );
